...
import { faChevronDown, faChevronRight, faQuestion } from '@fortawesome/free-solid-svg-icons'
...
export default function AbsensiEditor({isOpen, onClose, callBack, submitText = 'Tambah', title = 'Dzuhur', note = '', coordinates = {}}) {
    ...
    const [firstCoor, setFirstCoor] = useState(coordinates?.first || '')
    const [secondCoor, setSecondCoor] = useState(coordinates?.second || '')
                ...
                <div className='flex flex-col gap-2'>
                    <div className="flex gap-2 flex-col">
                        ...
                        <div className='flex gap-2 flex-col sm:flex-row'>
                            <input type="text" placeholder='Koordinat pertama' className='flex-1 shadow-lg shadow-primary/50 border-secondary rounded focus:ring-primary text-neutral-700' onChange={e => setFirstCoor(e.target.value)}  value={firstCoor}/>
                            <input type="text" placeholder='Koordinat kedua' className='flex-1 shadow-lg shadow-primary/50 border-secondary rounded focus:ring-primary text-neutral-700' onChange={e => setSecondCoor(e.target.value)}  value={secondCoor}/>
                        </div>
                    </div>
